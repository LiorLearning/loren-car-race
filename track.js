...
function generateTrackPath(trackWidth, grassWidth) {
    const trackPath = [];
    const trackPointCount = 500; // More points for a longer track
    
    // Create a long track with slight curves
    const length = 600; // Very long track
    const width = 150;
    
    for (let i = 0; i <= trackPointCount; i++) {
        const ratio = i / trackPointCount;
        const x = ratio * length - length/2; // Centered at origin
        
        // Apply slight curves using sin functions with different frequencies
        const z = Math.sin(ratio * Math.PI * 3) * 30 + // Medium frequency curves
                Math.sin(ratio * Math.PI * 1.5) * 40 + // Low frequency curves
                Math.sin(ratio * Math.PI * 6) * 10;   // High frequency small curves
