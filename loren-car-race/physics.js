...
export function updatePhysics(gameState) {
    ...
    if (gameState.moveForward) {
        const speed = gameState.carSpeed + 0.1;
        gameState.carSpeed = Math.min(speed, 1.0);
    } else if (gameState.moveBackward) {
        const speed = gameState.carSpeed - 0.1;
        gameState.carSpeed = Math.max(speed, -0.5);
    } else {
        // Apply friction
        gameState.carSpeed *= 0.95;
    }

    // Apply speed debuff to red car if active
    if (gameState.redCarSpeedDebuff > 0) {
        gameState.carSpeed *= (1 - gameState.redCarSpeedDebuff);
    }

    // Update blue car physics
    if (gameState.gameMode === 'multiplayer') {
        if (gameState.blueMoveForward) {
            const speed = gameState.blueCarSpeed + 0.1;
            gameState.blueCarSpeed = Math.min(speed, 1.0);
        } else if (gameState.blueMoveBackward) {
            const speed = gameState.blueCarSpeed - 0.1;
            gameState.blueCarSpeed = Math.max(speed, -0.5);
        } else {
            // Apply friction
            gameState.blueCarSpeed *= 0.95;
        }

        // Apply speed debuff to blue car if active
        if (gameState.blueCarSpeedDebuff > 0) {
            gameState.blueCarSpeed *= (1 - gameState.blueCarSpeedDebuff);
        }
    }
}
